...
def main():
    ...
    parser.add_argument('-n', '--thread-num', dest='thread_num', default=8, type=int, help='number of worker threads; also the hard cap on in-flight chunks and upload memory use [default: 8]')
    ...
    parser.add_argument('-s', '--chunk-size', dest='chunk_size', default="30MB", help='chunk size per upload in bytes; note: each in-flight chunk holds ~chunk_size in memory, with up to thread_num chunks when later chunks repeatedly overtake earlier ones. Smaller chunks waste less on retries over flaky networks [default: 30MB]')
